...
export const Breadcrumb = () => {
  ...
  const getLangFromPathname = (path: string): "en" | "bn" | null => {
    const first = path.split("/").filter(Boolean)[0];
    if (first === "en" || first === "bn") return first;
    return null;
  };
  ...
  // Don't show breadcrumb on homepage (including language homepages)
  if (pathname === "/" || pathname === "/en" || pathname === "/bn") return null;

  const generateBreadcrumbs = (): BreadcrumbItem[] => {
    const breadcrumbs: BreadcrumbItem[] = [];
    const rawSegments = pathname.split("/").filter(Boolean);
    const pathSegments = lang ? rawSegments.slice(1) : rawSegments;
